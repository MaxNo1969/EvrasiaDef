...
public:
	...
	bool hard;
	void Copy(CRListT* _p);
	void Sort(TListSortCompare _Compare);
	void Sort(void);
};
...
//---------------------------------------------------------------------------
template<class T>
void CRListT<T>::Copy(CRListT* _p)
{
	Clear();
	for (int i = 0; i < _p->L->Count; i++)
		L->Add(_p->L->Items[i]);
}
template<class T>
void CRListT<T>::Sort(TListSortCompare _Compare)
{
	L->Sort(_Compare);
}
template<class T>
void CRListT<T>::Sort(void)
{
	L->Sort(T::Compare);
}
#endif
