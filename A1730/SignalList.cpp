...
SignalList::SignalList(void)
{
	...
	{
		PA pi = pa[i];
		S.Add(new CSignal(pi.Item("Name").String(), pi.Item("Hint").String(), 0,
	...
	if (PA("Defect.PCIE1730_1").Exist())
	{
		...
		for (int i = 0; i < Count; i++)
		{
			...
			S.Add(new CSignal(pi.Item("Name").String(),
				pi.Item("Hint").String(), 1, pi.Item("Input").Bool(),
				pi.Item("Digital").Bool(), pi.Item("Digital").Bool() ? 16 :
				0 + pi.Item("Position").Int(), WriteSignals, cs, Wait));
			// AnsiString a = _section;
			// a += "[";
			// a += Key;
			// a += "] ";
...
CSignal* SignalList::FindZero(AnsiString _name, bool _in)
{
	for (int i = 0; i < S.Count(); i++)
	{
...
{
	CSignal* ret=FindZero(_name, _in);
	if(ret!=NULL)
		return(ret);
