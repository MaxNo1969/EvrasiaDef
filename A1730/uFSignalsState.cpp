...
//---------------------------------------------------------------------------
__fastcall TFSignalsState::TFSignalsState(TComponent* Owner, SignalList* _SL) : TForm(Owner)
{
	SL = _SL;
	space = 1;
}
...
void __fastcall TFSignalsState::FormClose(TObject *Sender, TCloseAction &Action)
{
...
void TFSignalsState::LoadSettings(void)
{
	bool byHint = PA("Defect.Some.SignalsHint").Bool();
	for (int i = 0; i < ControlCount; i++)
	{
		TControl* p = Controls[i];
		if (p->ClassName() != "TFRSigPanel")
...
//---------------------------------------------------------------------------
void __fastcall TFSignalsState::FormResize(TObject *Sender)
{
	int w = (ClientWidth - space * 3) / 2;
	int l = w + 2 * space;
	for (int i = 0; i < ControlCount; i++)
	{
		TControl* cn = Controls[i];
		TClass aa = cn->ClassType();
		AnsiString cname = cn->ClassType()->ClassName();
		if (cname != "TFRSigPanel")
			continue;
		TFRSigPanel* p = (TFRSigPanel*)cn;
		p->Width = w;
		if (!p->signal->IsInput())
			p->Left = l;
	}
