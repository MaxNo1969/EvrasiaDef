...
unsigned short Crc16::Compute(unsigned char* _data, int _length)
{
	unsigned short crc = 0xFFFF;
	for (int i = 0; i < _length; i++)
		crc = (crc >> 8) ^ CrcTable[(crc ^ _data[i]) & 0xFF];
	return crc;
}

void Crc16::Add(unsigned char* _data, int _length)
{
	unsigned short crc = Compute(_data, _length - 2);
	_data[_length - 2] = crc & 0xff;
	_data[_length - 1] = crc >> 8;
}

bool Crc16::Check(unsigned char* _data, int _length)
{
	unsigned short crc = Compute(_data, _length - 2);
//	int a1=crc & 0xff;
//	int a2=crc >> 8;
//	int b1=_data[_length - 2];
//	int b2=_data[_length - 1];
//	if (a1!=b1)
//		return (false);
//	if(a2!=b2)
//		return (false);
	if (_data[_length - 2] != (crc & 0xff))
		return (false);
	if (_data[_length - 1] != (crc >> 8))
		return (false);
	return (true);
}
